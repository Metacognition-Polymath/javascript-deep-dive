...
- HTML문서를 파싱한 결과물 => DOM
  - DOM에는 HTML문서 구조 뿐만아니라 스타일 등을 변경할 수 있는 프로그래밍 인터페이스로서 DOM API를 제공한다
  - 자바스크립트 코드에서 DOM API를 사용하면 이미 생성된 DOM을 동적으로 조작할 수 있다
  - 39장 DOM에서 자세히 알아보자
- CSS 파싱때와 마찬가지로 HTML을 한줄씩 파싱하며 DOM을 생성하다가
  script태그를 만나면 DOM 생성을 일시 중단한다
- script 태그의 src 어트리뷰트에 정의된 자바스크립트 파일을 서버에 요청하여 로드한 자바스크립트 파일을
  또는 script태그안의 자바스크립트 코드를 파싱하기 위해 자바스크립트 엔진에 제어권을 넘긴다
- 이후 자바스크립트 파싱과 실행이 종료되면 렌더링 엔진으로 다시 제어권을 넘겨 HTML파싱이 중단된 지점부터
  다시 HTML 파싱을 시작하여 DOM 생성을 재개한다
- 자바스크립트 파싱과 실행은 브라우저의 렌더링 엔진이 아닌 자바스크립트 엔진이 처리한다
  - 자바스크립트 엔진은 자바스크립트 코드를 파싱하여 CPU가 이해할 수 있는 저수준언어로 변환하고 실행하는 역할을 한다
- 자바스크립트 코드 파싱
  - JS를 해석
    => AST(Abstract Syntax Tree)를 생성
    => 인터프리터가 실행할 수 있는 중간코드 - 바이트코드 - 를 생성하여 실행
  - **토크나이징** : 소스코드를 분석하여 문법적 의미를 갖는 코드의 최소 단위인 토큰들로 분해한다
  - **파싱** : `AST를 생성`한다
    - AST는 토큰에 문법적 의미와 구조를 반영한 트리 구조의 자료구조다
  - **바이트 코드 생성과 실행**
    - AST를 바이트코드 생성기가 바이트코드로 변환하여
      인터프리터가 실행할 수 있게 해준다
...
- 리플로우 : 레이아웃을 다시 계산하는 것
  ...
  - 요소의 크기/위치 변경
  - 윈도우 리사이징 등
  - 레이아웃에 영향을 주는 변경이 발생한 경우에 한하여 실행 됨
- 리페인트 : 재결합된 렌더 트리를 기반으로 다시 페인트를 하는 것
  - 리플로우 없이 리페인트만 실행될 수도 있음

## 38.8 자바스크립트 파싱에 의한 HTML 파싱 중단

- script태그가 앞에 위치하면 파싱이 중단되고 JS코드를 실행하는데
  DOM이나 CSSOM이 아직 생성되지 않은 상태라면 문제가 발생할 수 있다
  - 때문에 body맨 아래쪽에 위치시키게 된다

## 38.9 script 태그의 async/defer 어트리뷰트

- body맨 아래쪽에 script태그를 위치시키면 문제가 될일은 없지만
- 브라우저가 js파일을 다운받는 것을 기다리고 그것을 다시 실행하게 되는데
- DOM 파싱하는 동안 js파일을 다운받을 수 있는 방법이 있다
  - async와 defer 어트리뷰트를 추가하는 것이다
  - async어트리뷰트는 파싱과 동시에 js파일을 다운받을 수 있지만 js파일 다운로드가 끝나면 바로 실행하기 때문에
    DOM파싱 전에 실행될 가능성이 있다
  - defer어트리뷰트는 DOM 생성이 완료된 직후(DOMContentLoaded 이벤트에 의하여) js소스가 실행된다
